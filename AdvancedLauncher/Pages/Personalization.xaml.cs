    ...
    public partial class Personalization : UserControl
    {
        ...
        private void SaveBtn_Click(object sender, RoutedEventArgs e)
        {
            if (isGameImageLoaded)  //Сохраняем только если картинка загружена
            {
                ...
                if (result == true)
                {
                    try { File.WriteAllBytes(sFileDialog.FileName, current_image_bytes); }
                    catch (Exception ex)
                    {
        ...
        private bool LoadGameImage(ResourceItemViewModel item)
        {
            ...
            bool IsOpened = false;
            try { IsOpened = dmoFS.Open(FileAccess.Read, 16, LauncherEnv.Settings.pCurrent.GameEnv.GetHFPath(), LauncherEnv.Settings.pCurrent.GameEnv.GetPFPath()); }
        ...
        private void BtnApply_Click(object sender, RoutedEventArgs e)
        {
            ...
            DMOFileSystem dmoFS = LauncherEnv.Settings.pCurrent.GameEnv.GetFS();
            bool IsOpened = false;
            try { IsOpened = dmoFS.Open(FileAccess.Write, 16, LauncherEnv.Settings.pCurrent.GameEnv.GetHFPath(), LauncherEnv.Settings.pCurrent.GameEnv.GetPFPath()); }
            catch { IsOpened = false; }
